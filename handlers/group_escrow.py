...
async def cmd_seller(message: Message, bot: Bot):
    ...
    if deal:
        if deal['buyer_id'] == user_id:
            return await message.answer("? You are already registered as the <b>Buyer</b> in this deal. You cannot switch roles.", parse_mode="HTML")
        if deal['seller_id'] == user_id:
            await db.execute("UPDATE group_deals SET seller_wallet = $1, currency = $2 WHERE chat_id = $3", wallet, currency, chat_id)
            await message.answer(f"? Seller wallet updated: <code>{wallet}</code>", parse_mode="HTML")
...
async def cmd_buyer(message: Message, bot: Bot):
    ...
    if deal:
        if deal['seller_id'] == user_id:
            return await message.answer("? You are already registered as the <b>Seller</b> in this deal. You cannot switch roles.", parse_mode="HTML")
        if deal['buyer_id'] == user_id:
            await db.execute("UPDATE group_deals SET buyer_wallet = $1, currency = $2 WHERE chat_id = $3", wallet, currency, chat_id)
            await message.answer(f"? Buyer wallet updated: <code>{wallet}</code>", parse_mode="HTML")
            await check_deal_ready(message, bot)
            return
